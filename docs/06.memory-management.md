...
void custom_arena_example(void) {
  ...
  Arena *custom_arena = arena_borrow();
  
  // Allocate from custom arena
  char *data1 = arena_strdup(custom_arena, "Hello");
  char *data2 = arena_sprintf(custom_arena, "Number: %d", 42);
  int *numbers = arena_alloc(custom_arena, sizeof(int) * 100);
  
  // Use the data
  char *response = arena_sprintf(custom_arena, "%s - %s", data1, data2);
  
  printf("%s\n", response);
  
  // Give it back to the pool
  arena_return(custom_arena);
}
```
