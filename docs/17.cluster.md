...
Enable cluster module in your `CMakeLists.txt` file, before the `FetchContent_MakeAvailable(ecewo)` line:
...
set(ECEWO_CLUSTER ON CACHE BOOL "Build cluster module" FORCE)
