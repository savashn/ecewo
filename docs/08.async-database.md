...
1. Enable `ecewo/postgres.h` module in your `CMakeLists.txt` file, before the `FetchContent_MakeAvailable(ecewo)` line:

```cmake
set(ECEWO_POSTGRES ON CACHE BOOL "Build PostgreSQL integration module" FORCE)
```

Or add `-DECEWO_POSTGRES=ON` flag when building.
