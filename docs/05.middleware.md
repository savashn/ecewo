...
int admin_only(Req *req, Res *res, Next next)
{
...
int validate_settings(Req *req, Res *res, Next next)
{
...
int auth_middleware(Req *req, Res *res, Next next)
{
...
int logging_middleware(Req *req, Res *res, Next next)
{
    printf("%s %s\n", req->method, req->path);
    next(req, res);
}
...
void auth_middleware(Req *req, Res *res, Next next)
{
    ...
    if (!token)
    {
        send_json(res, 401, "{\"error\":\"No token\"}");
        return;
    }
