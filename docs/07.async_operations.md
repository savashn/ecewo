...
2. [I/O Bound Async](#io-bound-async)
    1. [Async Postgres Querying](#async-postgres-querying)
...
> - Independent parallel blocking operations, use `worker()` function
> - Need fastest response? Use `worker()` for parallel execution
> - Limited worker threads? Use handler-level to conserve threads

## I/O Bound Async

### Async Database Queries

For asynchronous database queries, Ecewo officially supports [libpq](https://www.postgresql.org/docs/current/libpq.html) — the official PostgreSQL client library, which includes asynchronous support. It basically combines [libpq](https://www.postgresql.org/docs/current/libpq.html) and [libuv](https://libuv.org/) for using in Ecewo. See the [ecewo-postgres chapter](/docs/examples/async_postgres.md).

If you would like to use any other database, such as MySQL or MongoDB, refer to its own documentation for async behavior.

If you would like to use a database that has no async API, such as SQLite, you may still use the Ecewo's `worker` for blocking performance-critical big queries.
