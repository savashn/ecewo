...
There is `set_timeout()` function that executes a callback function for one-time after a specified delay.
...
There is `set_interval()` function that executes a callback function repeatedly at specified intervals.
...
There is `clear_timer()` function that cancels a created timer. It is useful for long-time processes.
