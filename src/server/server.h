...
// Configure thread pool size for async handlers
// Must be called before server_init()
// Default: UV_THREADPOOL_SIZE environment variable or 4
void server_set_thread_pool_size(int size);
...
static inline void get_async_with_mw(
    const char *path,
    MiddlewareArray mw,
    RequestHandler handler)
{
